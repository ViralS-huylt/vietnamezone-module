...
@section('title') {{_t('list_province')}} @endsection
...
@section('content')
    @component('common-components.breadcrumb')
        @slot('title') {{_t('list_province')}} @endslot
        @slot('li_1') {{ _t('home') }} @endslot
        @slot('li_2') {{_t('list_province')}} @endslot
    @endcomponent
                            ...
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <input type="text"
                                           ...
                                           value="{{ $search  }}"
                                           @endif
                                           onkeypress="return search(event, '{{ route('province.index') }}')"
                                    />
                                    <i class="bx bx-search-alt search-icon"
                                       onclick="return redirectWithSearch('{{ route('province.index') }}')"></i>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-8">
                            <div class="text-sm-right">
                                <a type="button" style="color: white;" href="{{ route('province.create') }}"
                                   class="btn btn-success btn-rounded waves-effect waves-light mb-2 mr-2"><i
                                        class="mdi mdi-plus mr-1"></i> {{ _t('add_new') . ' ' . _t('province') }}
                                </a>
                            </div>
                        </div><!-- end col-->
                    </div>
                        ...
                        <table class="table table-centered table-nowrap">
                            ...
                            @foreach($provinces as $province)
                                <tr>
                                    ...
                                    <td>{{ $province->name }}</td>
                                    <td>
                                        <span  class="badge {!! $province->active ? 'badge-success' : 'badge-danger' !!}">
                                            {!! $province->active ? _t('active_province') : _t('inactive_province') !!}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        ...
                                        {!! Form::open([
                                                'method' => 'DELETE',
                                                'route' => [ 'province.destroy', $province->id ],
